<template>
  <Article class="article-skills" :model="model">
    <div class="article-content-wrapper">
      <!-- Chart -->
      <div v-if="chartType" class="chart-wrapper">
        <!-- Chart Type: Pie -->
        <ArticleSkillsPieChart
          v-if="chartType === 'pie'"
          :model="props.model" />
      </div>
      ...
      <!-- Items -->
      <div
        class="items-row-wrapper"
        :class="chartType ? `items-row-wrapper-with-chart` : ``">
        <div class="row" :class="`${gutterClass}`">
          <ArticleSkillsItem
            v-for="item in props.model.items"
            :item="item"
            :col-class="chartType ? colClassWithChart : colClass"
            :max-items-per-line="Number(maxItemsPerLine)"
            :transparent-icon="
              props.model.getSetting('transparent_item_icon_background', false)
            "
...
const maxItemsPerLine = computed(() => {
  ...
  if (maxItemsPerLine < 0 || maxItemsPerLine > 12)
    throw new Error(
      "[ArticleSkills] The setting max_items_per_line must be a number between 1 and 12."
    );
