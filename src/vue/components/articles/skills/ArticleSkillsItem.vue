<template>
  <div class="skills-item" :class="colClass">
    <IconView
      class="skills-item-icon"
      :class="`skills-item-icon${transparentIcon ? '-transparent' : ''}${
        smallIcon ? '-shrink' : ''
      }`"
      :img="item.img"
    ...
    <div v-if="titleDisplay" class="skills-item-content">
      <!-- Header -->
      <div class="skills-item-header eq-h6">
        <!-- Title -->
        <span v-if="titleDisplay" class="skills-item-header-title">
          <span class="fw-bold" v-html="titleDisplay" />
        ...
        <!-- Percentage -->
        <span
          v-if="percentageDisplay && progressBarAllowed"
      ...
      <!-- Progress Bar -->
      <div
        v-if="item.hasPercentage && progressBarAllowed"
        class="skills-item-progress-display mt-1">
        <div
          class="skills-item-progress-display-fill"
          :style="`width:${item.percentage}%; background-color: ${item.fallbackFaIconColor}`" />
      </div>

      <!-- Experience Time -->
      <div
        v-if="experienceTimeDisplay && progressBarAllowed"
      ...
      <!-- Description -->
      <div v-if="titleDisplay" class="skills-item-description">
        <p
          class="text-3 m-0"
...
const percentageDisplay = computed(() => {
  if (!props.item.percentage) return;
  return props.item.percentage + "%";
});

const experienceTimeDisplay = computed(() => {
  const dateStart = props.item.dateStart;
  if (!dateStart || !(dateStart instanceof Date)) return null;

  const yearsDiff = utils.getYearsPassedSince(dateStart);
  const floorYearsDiff = Math.floor(yearsDiff);
  if (floorYearsDiff > 1)
    return localizeFromStrings("experience_year_count_plural").replace(
      "{x}",
      floorYearsDiff + "+"
    );
  else if (floorYearsDiff === 1)
    return localizeFromStrings("experience_year_count_singular").replace(
      "{x}",
      floorYearsDiff
    );
  else return localizeFromStrings("experience_year_count_less_than_one");
});
...
const titleDisplay = computed(() => {
  const title = localize(props.item.locales, "title", true);
  return title && !title.includes("locales.") ? title : null;
});
